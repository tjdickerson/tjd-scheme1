...
-- Get the directory where the tjdscheme plugin is located
local function get_plugin_dir()
  return debug.getinfo(1).source:match('@?(.*/)'):gsub('/lua/tjdscheme/$', '')
end
...
-- Turn tjdscheme naming conventions into more common ANSI names
local function get_palette(variant)
  package.loaded['tjdscheme/palettes/' .. variant] = nil
  local colors = require('tjdscheme/palettes/' .. variant)
  -- stylua: ignore
  return {
    bg             = colors.a.bg,
    fg             = colors.a.fg,
...
local function build(terminals)
  for _, variant in ipairs { 'dark', 'light' } do
    ...
    for term, attrs in pairs(terminals) do
      ...
      fwrite(fmt, string.format('%s/tjdscheme_%s%s', dir, variant, attrs.ext))
    end

    fwrite(generate_iterm2(palette), string.format('%s/term/tjdscheme_%s.itermcolors', get_plugin_dir(), variant))
    fwrite(vim.json.encode(palette), get_plugin_dir() .. string.format('/tjdscheme_%s.json', variant))
  end
end
